...
if(isset($_GET['action'])) {
	...
	if($action=="netcheck"){
        ...
        {       
                ...
                else if($rs[imei_mode]==4) $imei_ck5='selected';
	...
	elseif($action=="savemodify")
	{
		...
		if($sim_rs['limit_sms']!=$_POST['limit_sms']) $t_remain.=",remain_sms=$_POST[limit_sms]";
		if($sim_rs['month_limit_time']!=$_POST['month_limit_time']) $t_remain.=",month_remain_time=$_POST[month_limit_time]";
		if($sim_rs['no_connected_limit']!=$_POST['no_connected_limit']) $t_remain.=",no_connected_remain=$_POST[no_connected_limit]";
		if($dev_disable==0 && $sim_rs['dev_disable']==1) $t_remain.",no_ring_remain==if(),no_answer_remain=0,short_call_remain=0";
...
*/
		...
		if(!$team_id){
			...
			$send[]=my_pack2(DEV_BINDING, $sim_name, 0);
                ...
                sendto_xchanged($send);
                WriteSuccessMsg("<br><li>Modify SIM Slot successful!</li>","all_sim.php?bank_name=$_REQUEST[bank_name]&group_id=$_REQUEST[group_id]&order=$order_type&order_key=$order_key");
        }
	...
	elseif($action=="savemoremodify")
	{
		...
		if($_POST[time_limit] == "" || $_POST[time_limit] < -1) $_POST[time_limit]=-1;
		if($_POST[limit_sms] == "" || $_POST[limit_sms] < -1) $_POST[limit_sms]=-1;
		...
		if($ErrMsg!="")
			...
			if($_POST['month_limit_time_modify']) $sql.="month_limit_time='$_POST[month_limit_time]',month_remain_time='$_POST[month_limit_time]',";
			if($_POST['month_reset_day_modify']) $sql.="month_reset_day='$_POST[month_reset_day]',";
			if($_POST['time_limit_modify']) $sql.="time_limit='$_POST[time_limit]',remain_time='$_POST[time_limit]',";
			if($_POST['count_limit_modify']) $sql.="count_limit='$_POST[count_limit]',count_remain=$_POST[count_limit],";
			if($_POST['limit_sms_modify']) $sql.="limit_sms='$_POST[limit_sms]',remain_sms='$_POST[limit_sms]',";
			if($_POST['count_limit_no_connect_modify']) $sql.="count_limit_no_connect='$_POST[count_limit_no_connect]',";
			if($_POST['no_connected_limit_modify']) $sql.="no_connected_limit='$_POST[no_connected_limit]',no_connected_remain=$_POST[no_connected_limit],";
			...
			if($_POST['auto_reset_remain_modify']) $sql.="auto_reset_remain='$_POST[auto_reset_remain]',";
			if($_POST['auto_reset_remain_s_modify']) $sql.="auto_reset_remain_s='$_POST[auto_reset_remain_s]',";
...
/*
				...
				if($row['month_remain_time']==0) $row['dev_disable']=1;
				$send[]=my_pack($row, SIM_ADD);
				if($_POST['dev_disable_modify'] || $_POST['month_limit_time_modify']) $send[]=my_pack2($row[dev_disable]?DEV_DISABLE:DEV_ENABLE, $row[sim_name], TYPE_SIM);
				if($row['imei_mode']==3) $send[]=my_pack($row, IMEI_IMSI_INFO);
*/
				if($_POST['team_id_modify'] && $team_id) $send[]=my_pack2(DEV_BINDING, $row[sim_name], 0);
		...
		$where=gen_sim_where();
		$db->query("update sim set no_ring_remain=0,no_answer_remain=0,short_call_remain=0,remain_time=time_limit,remain_sms=limit_sms,count_remain=count_limit,no_connected_remain=no_connected_limit,month_remain_time=month_limit_time $where");
...
*/
	...
	else if($action=="set_imei_db"){
		$where=gen_sim_where();
		
		$query=$db->query("select sim.*,password from sim left join sim_bank on sim.bank_name=sim_bank.name $where");
		$num=$db->num_rows($query);
		$query0=$db->query("select id,imei from imei_db order by used limit $num");
		while($row0=$db->fetch_array($query0)) {
			$imei_db[]=$row0;
		}
		$i=0;
		while($row=$db->fetch_array($query)) {
			if($imei_db[$i]){
				$row['imei_mode']==2;
				$row['imei']=$imei_db[$i]['imei'];
				$imei_db[$i]['sim_name']=$row['sim_name'];
				$log.="<br>SIM Slot:$row[sim_name] Set IMEI:$row[imei]";
				$send[]=my_pack($row, SIM_ADD);
				$db->query("update imei_db set sim_name='$row[sim_name]',used=used+1,used_time=now() where id=".$imei_db[$i]['id']);
				$db->query("update sim set imei_mode=2, imei='$row[imei]' where id=$row[id]");
			}
			else {
				$log.="<br>SIM Slot:$row[sim_name] not find IMEI from database";
			}
			$i++;
		}
		sendto_xchanged($send);
		WriteSuccessMsg("<br><li>The commond is sended to Server.$log</li>","?bank_name=$_REQUEST[bank_name]&group_id=$_REQUEST[group_id]&order=$order_type&order_key=$order_key");
	}
	//else $action="main";
...
*/
		...
		$fenye=showpage2("?bank_name=$_GET[bank_name]&group_id=$_GET[group_id]&order_key=$order_key&order=$order&",$page,$count,$perpage,true,true,"row(s)","myform","boxs");
		if($order_key) $orderby=" ORDER BY `$order_key` $order_type,sim.sim_name ";
		else $orderby=" ORDER BY sim.sim_name ";
		if($action=="export"){
			export_sim($where, $orderby);
		}
...
*/
			...
			else if($row['imei_mode']==4) $row['imei_type_name']="Database with IMSI";
			...
			$row['remain_limit'].="total remain SMS: ".$row['remain_sms']."/".$row['limit_sms']."\n";
			$row['remain_limit'].="period remain time: ".$row['period_time_remain']."\n";
			$row['remain_limit'].="period remain count: ".$row['period_count_remain']."\n";
			$row['remain_limit'].="period remain SMS: ".$row['period_remain_sms']."\n";
			$row['remain_limit'].="month remain time: ".$row['month_remain_time']."";
			...
			if($row['remain_time']==0 || $row['period_time_remain']==0 || $row['period_count_remain']==0 || $row['count_remain']==0 || $row['no_connected_remain']==0 || $row['remain_sms']==0 || $row['period_remain_sms']==0 || $row['month_remain_time']==0){
				$row['remain_state'] = '<font color="#FF0000">Exhaust</font>';
			}
			else if(($row['remain_time']<=$limit_t_w && $row['remain_time']!=-1) || ($row['period_time_remain']<=$limit_t_w && $row['period_time_remain']!=-1)|| ($row['period_count_remain']<=$limit_c_w && $row['period_count_remain']!=-1) || ($row['count_remain']<=$limit_c_w && $row['count_remain']!=-1) || ($row['no_connected_remain']<=$limit_c_w && $row['no_connected_remain']!=-1) || ($row['remain_sms']<=$limit_c_w && $row['remain_sms']!=-1) || ($row['period_remain_sms']<=$limit_c_w && $row['period_remain_sms']!=-1) || ($row['month_remain_time']<=$limit_t_w && $row['month_remain_time']!=-1)){
