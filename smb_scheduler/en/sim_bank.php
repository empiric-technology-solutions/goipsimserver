...
if(isset($_GET['action'])) {
	...
	if($action=="del")
	{
		...
		if($ErrMsg!="")
			...
			WriteSuccessMsg("<br><li>Delete successful</li>","sim_bank.php");
	...
	elseif($action=="saveadd")
	{
		...
		if($no_t[0])
			$ErrMsg	.='<br><li>This ID ('.$name.') already exist or may conflict whit other type.</li>';
		if($ErrMsg!="")
			WriteErrMsg($ErrMsg);
		else{
			...
			//sendto_cron(); 
			WriteSuccessMsg("<br><li>Add successful</li>","sim_bank.php");				
		}
	}
	elseif($action=="savemodify")
	{
		...
		if($ErrMsg!="")
			...
			WriteSuccessMsg("<br><li>Modify successful</li>","sim_bank.php");
		}
	}
